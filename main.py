...
class CleanDataset():
    def __init__(self, config):
        ...
        self.adj = np.load(config.data.spatial)#加载空间邻接矩阵
        self.label, self.feature = self.read_data()
        self.spatial_distance = search_multihop_neighbor(self.adj, hops=self.alpha)
        self.range_mask = self.interaction_range_mask(hops=self.alpha, t_size=self.t_size)#表示目标节点可以访问的最大空间跳变距离

    def read_data(self):
        if 'PEMS' in self.data_name:
            data = np.expand_dims(np.load(self.feature_file)[:,:,0],-1)#(17856,170,3)->(17856,170,1)
        else:
    ...
    def interaction_range_mask(self, hops = 2, t_size=3):
        ...
        hop_arr[hop_arr != -1] = 1  # 将所有非 -1 的元素设置为 1
        hop_arr[hop_arr == -1] = 0 # 将所有 -1 的元素设置为 0
...
def main(config):
    ...
    loss_function =nn.L1Loss()#绝对值损失函数
...
def run(args):
    ...
    config.model.early_stop = args.early_stop# 早停阈值
    config.model.end_epochs = args.max_epoch

    config.model.L      = args.L # Number of layer
    config.model.C      = args.C # Dimension of node features 节点特征维度
    config.model.d      = args.d # Dimension of position embedding 位置嵌入维度
    ...
    config.model.range_mask = clean_data.range_mask#表示目标节点可以访问的最大空间跳变距离
    config.model.spatial_distance = clean_data.spatial_distance#空间距离矩阵
